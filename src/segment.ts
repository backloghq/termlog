 ...
 *   [doc-length sidecar]    — uint32 docCount, then uint32 per doc length
 ...
 *   4  bytes  version        = SEGMENT_VERSION
...
export class SegmentCorruptionError extends Error {
  constructor(public readonly region: "postings" | "sidecar" | "dict" | "footer", detail: string) {
...
export class SegmentWriter {
  ...
  /**
   * Serialize and write to backend as `<id>.seg` via atomic `.seg.tmp` rename.
  ...
  async flush(id: string, backend: StorageBackend): Promise<string> {
    ...
    const dictOffset = sidecarOffset + sidecarBuf.length;
...
export class SegmentReader {
  ...
  static async open(path: string, backend: StorageBackend): Promise<SegmentReader> {
    ...
    const footer: SegmentFooter = {
      ...
      dictOffset, dictLength, postingsCrc32, sidecarCrc32, dictCrc32,
      docCount, termCount,
    };
    ...
    return new SegmentReader(postingsRegion, dict, docLenMap, footer.docCount, footer.termCount);
  }
